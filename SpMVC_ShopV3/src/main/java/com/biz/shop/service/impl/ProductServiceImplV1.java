...
public class ProductServiceImplV1 implements ProductService{
	...
	public int update(ProductVO vo) {
		...
		// 업데이트를 수행하기 전에 삭제 Flag칼럼을 강제로 null로 설정하여 해당상품이 삭제표시가 되지 않도록한다.
	 ...
	 * Master Data는
	 * Work Data(매입매출, 거래처외상정보, 회원로그인로그와 같은)의
	 * 기준이 되는 값을 가지는 Data Table이다.
	 * 
	 * 어플을 사용해서 어떤 업무를 진행하는 과정에서
	 * WorkData에 저장되는 데이터는 필수적으로 Master Data와 Join관계가 맺어진다.
	 * 만약 WorkDat와 Master Data를 Join하여 어떤 통계정보를 찾고자할때 Master에 해당하는 Data(코드와 일치하는)가
	 * 없으면 WorkData의 정보가 어떤내용인지 확인이 어려워 지는경우가 많다.
	 * 
	 * 통상적으로 Master Data는 원칙상 PK 칼럼 값은 변경하지 않고 또한 한번 insert된 Master Data는 삭제하지 않는다.
	 ...
	 * 2. VO의 p_not_use 칼럼을 null이 아닌 값으로 셋팅하여
	 * 3. update를 수행한다.
	...
	public String getPCode() {
		 ...
		 * P0001, P0002, P0010 이런 코드가 있다고 가정하면 그 중에 P0010코드를 가져오는 SQL을 만들겠다.
		 */
		 ...
		 * retPCode를 생성하는 코드를 실행하여 try에서 exception이 발생하여 건너뛰도록 한다.
