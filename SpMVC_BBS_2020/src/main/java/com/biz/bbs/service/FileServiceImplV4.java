...
/*
 * 파일 upload 프로젝트에서 외부의 파일을 공개하기위해 폴더를 개방해두면
 * 폴더를 외부에서 접근하여 해킹을 수행하는 경우들이 발생할 수 있다.
 * 
 * 특히, 프로젝트 구조를 알게되는 경우 같은 이름의 파일을 엉뚱한 파일로 upload하여
 * 원래 저장되어있던 파일을 변형하는 일들이 발생할 수 있다. (ex. Image Injection)
 * 
...
public class FileServiceImplV4 extends FileServiceImplV1{
	...
	/*
	 * 필드(멤버)변수를 private final로 선언했을 경우 보통 final로 선언된 변수는
	 * 선언과 동시에 생성(초기화)를 해야한다.
	 * private final로 선언된 멤버변수는 클래스의 생성자 method에서 초기화하는 것을 허용한다.
	 * 
	 * private final로 선언된 멤버변수는 반드시 클래스의 생성자 메서드에서 초기화를 해야한다.
	 */
	
	/*
	 * private으로 선언된 rootFolder 변수를 protected로 변경
	 * protected로 선언된 변수들을 현재클래스를 상속받은 클래스에서 그대로 사용이 가능하다.
	 */
	...
	public String fileUp(MultipartFile file) {
		 ...
		 * 		만약 해커가 해당 파일이름을 알게되어 동일한 이름의 파일을 만들어 UpLoad하면
