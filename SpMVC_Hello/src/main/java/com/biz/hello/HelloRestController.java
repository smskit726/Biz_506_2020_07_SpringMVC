...
/*
 * @RestController
 * SpringFrameWork 4.x이상에서 지원되는 새로운 Controller
 */
@RestController
public class HelloRestController {
	 ...
	 * Container가 있으며 컨테이너에 저장된 객체를 찾아서 선언된 객체에 주입하여 초기화, 사용할 수 있도록 만들어준다.
...
//	}
	...
	/*
	 * Spring 프로젝트에서는 외부의 클래스를 객체로 만들때 직접 new 생성자를 사용하여 만들지 않는다
	 * 
	 * 프로젝트가 시작될 때 @Annotation이 붙어있는 모든 클래스는 이미 객체로 생성되어
	 * Container에 저장되어 있다.
	 * 
	 * 다른 클래스를 객체로 생성하여 사용이 필요한 곳이 있으면 Container에서 객체를 꺼내서 직접 주입해 준다.
	 * 
...
//	}
	...
	public String nNull(Model model) {
	 ...
	 * 
	 * 이 method에서 intList = new ArrayList<Integer>(); 를 생략하면
	 * List<Integer> intList = null; 에서 intList가 선언은 되었지만 초기화가 되지 않아서
	 * 
