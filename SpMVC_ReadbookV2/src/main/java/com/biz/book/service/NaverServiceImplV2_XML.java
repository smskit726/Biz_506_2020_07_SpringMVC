...
public class NaverServiceImplV2_XML extends NaverServiceImplV1{
	...
	// NaverServiceV1의 3개의 method를 모두 상속받아서 여기에 아무런 코드가 없어도 프로젝트가 정상 수행이 되고 있다.
	
	// 3개의 method 중에서 getNaverList(jsonString) method만 upgrade 하겠다.
	@Override
	public List<BookVO> getNaverList(String queryURL) {
		
		// queryURL(요청주소포함된) 문자열을 URI 객체로 만들어 http 프로토콜에서 사용할 수 있도록 만드는 클래스
		// 기존의 URL 클래스가 있으나, 새로운 기능을 수행하기 위해서 별도로 URI클래스를 만들어 놨으며
		// 많은 기능들이 추가되어 있다.
		// RestTemplate를 사용하기 위해서는 queryURL문자열을 URL 객체가 아닌 URI 객체로 사용해야 한다.
		URI restURI = null;
		
		/*
		 * springFrameWork에서 외부 API를 사용하여 데이터를 가져올때 기존에는 JSON(XML)형식으로 가져오고, 여러가지 외부 라이브러리를 사용하여
		 * 객체로 parsing하는 과정을 복잡하게 만들어야 했다.
		 * Spring에서 외부 API를 사용하여 데이터를 가져올일이 점점 많아 지면서 RestTemplate라는 클래스를 새로 만들어
		 * frameWork 프로젝트에서 사용할수 있도록 만들어 두었다.
		 * 
		 * HttpHeader, HttpEntity, ResponseEntity 객체만 잘 작성하면 외부 API에 요청하고, 응답받은 데이터를 Parsing 하는 절차를 대신 수행해준다.
		 */
