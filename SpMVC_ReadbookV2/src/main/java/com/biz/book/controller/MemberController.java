 ...
 * 서버의 메모리에 memberVO이름으로 객체변수를 마련해 놓는다.
 * 이 변수는 서버가 재 시작하거나 하더라도 유지가 되는 성질이 있고 클라이언트에서
 * URL Request를 수행했을 때 메모리에 계속 유지된다.
...
public class MemberController {
	...
	/*
	 * @SessionAtrributes("memberVO")를 사용하려면 반드시 memberVO를 생성하는 method가 클래스에 있어야된다.
	 * UserDetailsVo 클래스로 생성된 memberVO가 "memberVO" 이름으로 보관된다.
	 * @SessionAtrributes() 잇는데 @ModelAttribute(가 붙은 method가 없으면 컴파일 에러가 난다.
	 */
	 ...
	 * 1. form에서 POST로 데이터를 보냈을 경우 form에서 보낸 데이터가 담긴 VO객체를 생성하여 method 내의 코드에서 사용할 수 있도록 준비해준다.
	 * 2. 아무도(아무곳에서도) 매개변수 클래스와 일치하는 변수를 전달하지 않을 경우 자체적으로 클래스의 생성자를 호출하여 비어있는 객체를 만들어서
...
//		이 코드를 @ModelAttribute("memberVO")가 대신한다
	 ...
	 * @ModelAttribute("memberVO") 설정을 확인하고 server에 임시보관중인 SessionAttribute("memberVO")를 찾아서
	 * 입력박스로부터 전달된 데이터를 보관한다.
	 * member-write2.jsp를 열고 나머지 데이터를 추가 입력한 후 join_comp POST로 보내면 먼저 입력받아서 SessionAttribute에 보관중인
	 * id, 비밀번호과 나중에 입력한 이름, 전화번호 등등 과 함께 묶어서 join_comp userVO에 담아준다.
	 * 입력폼의 항목이 매우 많을 때 입력폼을 분리해서 코딩을 해도 SessionAttributes의 성질을 이용하여 마치 입력마법사와 같은 기능을 구현할 수 있다.
	 */
...
//		memberService.insert(userVO);
	...
	/*
	 * @SessionAttributes() 사용할때는 DB에 데이터를 insert, update를 최종수행하고 나면 SessionStatus클래스의
	 * setComplete() method를 호출하여 서버에 남아 있는 메모리를 clear해 주어야 한다.
	 */
	...
	public String id_check(String username) {
		...
		// 문법오류가 발생하면 구체적으로 memberService에 method를 구현하는 방법
