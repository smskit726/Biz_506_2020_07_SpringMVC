...
/*
 * @Transactional Annotation을 클래스 차원에서 설정하면 현재 클래스의 모든 method에서
 * DB와 연동되는 부분이 자동으로 Transaction이 작동된다
 */
...
public class BooksController {
	 ...
	 * DataSourceTransactionManger 이것을 mybatis-context.xml에 만들어주고
	 * appServlet/tx-context.xml을 만들고 <tx:annotation-driven/>을 설정하고
	 * transaction이 필요한 method에 @Transactional Annotation을 설정해주면 자동으로
	 * 기본정책으로 Transaction이 수행된다.
	 */
...
//	@ResponseBody
	...
	/*
	 * spring form taglib 를 사용하여 write form을 만들었을 경우에는
	 * VO 클래스, 객체를 매개변수로 사용할때
	 * @ModelAttribut("VO")를 필수로 사용하자
	 */
	...
	public String input(@ModelAttribute("bookVO") BookVO bookVO, SessionStatus status) {
		...
		/*
		 * SessionAttribute를 Controller에 설정했을 경우
		 * 입력박스에 담긴값을 POST받아 DB에 반영한 후에는 반드시 SessionStatus.setComplete() method를 호출해서
