...
public class AuthProviderImpl implements AuthenticationProvider{
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		// username = "admin" or "user1" or "guest" 중에 하나이면 true
		...
		// 사용자ID 검사
		if(!bUser) {
			String msg = String.format("[ %s ] 아이디를 확인하세요", username);
			/*
			 * throw new Exception(message)
			 * throw : 강제로 exception을 발생시켜라
			 * spring security login이 진행되는 도중에 어떤문제가 발생을 하면
...
//		log.debug("USER VO : " + userVO.isAccountNonExpired());
		...
		if(!userVO.isAccountNonLocked()) {
			throw new LockedException("사용자 계정이 잠겨 있습니다</br>관리자에게 문의하세요");
		}
